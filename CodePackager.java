...
class SourceCodeFile {
	public static final String startMarker = "//:", // Start of source file
			...
			beginContinue = "} ///:Continued", endContinue = "///:Continuing", packMarker = "###", // Packed
																									// file
																									// header
																									// tag
	...
	// The file n ame separator from the old system:
	...
	public SourceCodeFile(BufferedReader pFile) {
		try {
			...
			System.out.println("l isting: " + dirname + filesep + filename);
...
class DirMap {
	...
	public void write() {
		...
		while (e.hasMoreElements()) {
			...
			IO.close(IO.dosOpen(new File(new File(rootpath, dir), Integer.toString(v.size()) + ".files")));
		}
	}
}
